...
test("requires an explicit data directory and forwards durable hub paths", () => {
  ...
  assert.equal(options.runtime.homeArtifacts.path, "/tmp/hob-agent-data/artifacts.sqlite");
  assert.equal(options.runtime.homeAuthorityCandidates.path, "/tmp/hob-agent-data/authority-candidates.sqlite");
  ...
  assert.notEqual(options.runtime.homeArtifacts.path, ":memory:");
  assert.notEqual(options.runtime.homeAuthorityCandidates.path, ":memory:");
