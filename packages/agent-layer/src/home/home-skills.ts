...
export function apply(ctx: Context): void {
  ...
  ctx.skills.register({
    ...
    content: [
      ...
      "When causality attributes the event to external-rule, call get_home_automation_trace with the same exact hwCapabilityId and provenance before answering. Treat automation trace status complete, partial, unknown, or unavailable exactly as returned; state missing run detail as unknown and do not guess.",
      ...
      "6. Hardware suggestions may name only the allowed sensing capability. Never name a brand, product, store, camera, or microphone. Explain necessity, placement, privacy impact, and a no-purchase alternative. With partial or insufficient confidence, hardware is optional and a reversible validation trial is required before purchase; recommended hardware requires sufficient evidence.",
      "For safety-related devices, use only manufacturer-approved test controls and procedures. Never advise placing powered equipment in water, deliberately creating a hazard, bypassing protection, or changing a valve or rule as part of a test.",
      "7. Publish exactly one report_home_advice result in the same language as the household question. Use plain household language: do not expose opaque IDs, schema names, semantic-kind labels, internal status codes, bridge names, or implementation jargon. Separate findings from unknowns, state confidence, include at most one bounded trial, and explain how the household can validate it.",
