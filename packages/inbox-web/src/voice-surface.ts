...
export type PrivateVoiceAvailability =
  | { readonly status: "active"; readonly recognitionMode?: "final_only" | "partial" }
...
export function renderVoiceSurface(
  ...
  const recognitionMarkup =
    ...
    (privateVoice.recognitionMode === "final_only" || privateVoice.recognitionMode === "partial")
      ? `<p class="product-muted product-voice-recognition" data-voice-recognition-mode="${privateVoice.recognitionMode}">${privateVoice.recognitionMode === "partial" ? "说话时会逐步显示当前听到的内容。" : "这次会在你说完后显示完整文字。"}</p>`
      : "";
