...
.product-media-completion-notification { position: relative; z-index: 19; display: flex; flex-wrap: wrap; align-items: center; justify-content: center; gap: .5rem 1rem; min-height: 2.75rem; padding: .65rem max(1rem, env(safe-area-inset-right)) .65rem max(1rem, env(safe-area-inset-left)); border-block-end: 1px solid var(--shell-rule); background: var(--shell-amber-soft); color: var(--shell-text); }
.product-media-completion-notification a { display: inline-flex; align-items: center; min-height: 2.75rem; color: var(--shell-blue); font-weight: 650; }
...
.product-host-view-switcher nav[data-host-view-shortcuts] a { min-height: 2.75rem; display: inline-flex; align-items: center; gap: .35rem; padding: .35rem .7rem; border-radius: 999px; color: var(--shell-muted); text-decoration: none; font-size: .82rem; }
...
.product-host-view-menu summary { min-height: 2.75rem; max-width: 100%; display: inline-flex; align-items: center; gap: .55rem; padding: .42rem .8rem; border: 1px solid var(--shell-rule); border-radius: 999px; background: var(--shell-bg); color: var(--shell-muted); cursor: pointer; list-style: none; }
...
.product-composer { display: grid; grid-template-columns: minmax(0, 1fr) auto auto auto; gap: .5rem; align-items: center; margin-block-start: 1.3rem; padding: .55rem; border: 1px solid var(--shell-rule); border-radius: 999px; background: var(--shell-surface); box-shadow: var(--shell-shadow-soft); }
...
.product-composer .product-voice-entry { min-height: 2.65rem; display: inline-flex; align-items: center; padding: .5rem .8rem; border: 1px solid var(--shell-rule); border-radius: 999px; color: var(--shell-muted); text-decoration: none; font-weight: 700; white-space: nowrap; }
.product-composer .product-voice-entry:hover { border-color: var(--shell-blue); color: var(--shell-blue); }
...
.product-composer .product-media-command-action, .product-conversation-composer .product-media-command-action { min-height: 2.65rem; padding: .52rem 1rem; border: 1px solid var(--shell-rule); border-radius: 999px; background: var(--shell-surface); color: var(--shell-blue); font-weight: 760; }
.product-composer .product-media-command-action:hover:not(:disabled), .product-conversation-composer .product-media-command-action:hover:not(:disabled) { border-color: var(--shell-blue); background: var(--shell-blue-soft); }
.product-media-command-action:disabled, .product-composer button:disabled, .product-conversation-composer button:disabled { cursor: not-allowed; opacity: .52; }
...
.product-runtime-outcome--done { margin: .3rem 0 0; color: var(--shell-green); font-weight: 650; }
.product-activity-verification { margin: .15rem 0 0; color: var(--shell-muted); font-size: .84rem; }
...
.product-turn-proposal { display: grid; gap: .4rem; margin-block-start: .6rem; padding: .7rem .85rem; border-radius: .8rem; background: var(--shell-blue-soft); font-size: .88rem; }
.product-turn-proposal p { margin: 0; }
.product-wall-approvals { margin-block-start: 1.15rem; }
.product-wall-approvals ul { margin: .4rem 0; padding: 0; list-style: none; display: grid; gap: .4rem; }
.product-wall-approvals li { display: flex; justify-content: space-between; gap: .8rem; padding-block: .35rem; border-block-end: 1px dashed var(--shell-rule); }
.product-safety-banner[data-safety-status="acknowledged"], .product-safety-banner[data-safety-status="contained"] { padding-block: .4rem; }
.product-safety-banner[data-safety-status="acknowledged"] .product-safety-copy span, .product-safety-banner[data-safety-status="contained"] .product-safety-copy span { display: none; }
...
.product-shell[data-connection-state="disconnected"] .product-metric-value { color: var(--shell-muted); }
.product-review-card .product-card-actions { display: flex; flex-wrap: wrap; gap: .55rem; margin-block-start: .3rem; }
.product-primary-action, .product-secondary-action, .product-danger-action { display: inline-flex; align-items: center; justify-content: center; min-height: 2.75rem; padding: .55rem .95rem; border-radius: .8rem; font-weight: 740; text-decoration: none; }
...
.product-quiet-action { min-height: 2.55rem; padding: .48rem .82rem; border: 1px solid var(--shell-rule); border-radius: .8rem; background: var(--shell-surface); color: var(--shell-ink); font-weight: 730; }
.product-quiet-action:hover { border-color: var(--shell-blue); color: var(--shell-blue); }
.product-action-form--stop .product-quiet-action { border-color: color-mix(in srgb, var(--shell-red) 32%, var(--shell-rule)); color: var(--shell-red); }
...
.product-conversation-composer { display: grid; grid-template-columns: minmax(0, 1fr) auto auto auto; gap: .55rem; margin-block-start: 1rem; padding-block-start: .9rem; border-block-start: 1px solid var(--shell-rule); }
...
.product-media-action-turn { display: grid; gap: .7rem; }
.product-media-action-turn h2, .product-media-action-turn p { margin: 0; }
.product-media-action-turn--pending { padding-inline-start: .85rem; border-inline-start: .25rem solid var(--shell-amber); }
.product-media-action-turn--verified { padding-inline-start: .85rem; border-inline-start: .25rem solid var(--shell-green); }
.product-media-action-turn--attention { padding-inline-start: .85rem; border-inline-start: .25rem solid var(--shell-amber); }
.product-media-clarification-options { display: grid; gap: .45rem; margin: 0; padding: 0; list-style: none; }
.product-media-clarification-options li { display: flex; flex-wrap: wrap; gap: .3rem .55rem; align-items: baseline; padding: .65rem .75rem; border: 1px solid var(--shell-rule); border-radius: .75rem; background: var(--shell-bg); }
.product-media-clarification-options li strong { flex: 1 1 12rem; }
.product-media-clarification-options li span { color: var(--shell-muted); font-size: .84rem; }
...
.product-control-workspace { overflow: hidden; border-block: 1px solid var(--shell-rule); background: var(--shell-surface); }
.product-control-summary { display: flex; align-items: center; gap: 1.4rem; padding: .9rem 1.1rem; border-block-end: 1px solid var(--shell-rule); background: var(--shell-sidebar); }
.product-control-summary > span { min-width: 5.5rem; display: grid; gap: .08rem; }
.product-control-summary strong { font-size: 1.15rem; font-variant-numeric: tabular-nums; }
.product-control-summary small, .product-control-summary p { color: var(--shell-muted); font-size: .78rem; }
.product-control-summary p { margin: 0 0 0 auto; max-width: 38rem; }
.product-control-spaces { display: grid; }
.product-control-space { padding: 1.1rem; border-block-end: 1px solid var(--shell-rule); }
.product-control-space:last-child { border-block-end: 0; }
.product-control-space > header { display: flex; align-items: center; justify-content: space-between; gap: 1rem; padding-block-end: .8rem; }
.product-control-space h2 { margin: 0; font-size: 1.08rem; letter-spacing: -.018em; }
.product-control-state { color: var(--shell-muted); font-size: .78rem; }
.product-control-metrics { display: flex; align-items: center; gap: 1rem; }
.product-control-metric { display: grid; justify-items: end; gap: .05rem; }
.product-control-metric small { color: var(--shell-muted); font-size: .72rem; }
.product-control-metric strong { font-variant-numeric: tabular-nums; }
.product-control-rows { border-block-start: 1px solid var(--shell-rule); }
.product-control-row { min-height: 3.5rem; display: grid; grid-template-columns: minmax(10rem, 1fr) minmax(5rem, auto) minmax(7rem, auto); align-items: center; gap: 1rem; margin: 0; padding-block: .45rem; border-block-end: 1px solid var(--shell-rule); }
.product-control-row:last-child { border-block-end: 0; }
.product-control-identity { min-width: 0; display: grid; gap: .08rem; }
.product-control-identity strong { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.product-control-identity small { color: var(--shell-muted); font-size: .72rem; }
.product-control-current-value { justify-self: end; color: var(--shell-ink); font-size: .86rem; font-variant-numeric: tabular-nums; }
.product-control-row .product-control-action { min-width: 7rem; min-height: 2.75rem; }
.product-control-workspace[data-control-row-density="compact"] .product-control-row { min-height: 2.85rem; padding-block: .18rem; }
.product-control-workspace[data-control-row-density="compact"] .product-control-row .product-control-action { min-height: 2.35rem; }
.product-control-empty { margin: 0; padding-block: .8rem .2rem; color: var(--shell-muted); }
...
.product-settings-sheet { max-width: 64rem; border-block: 1px solid var(--shell-rule); background: var(--shell-surface); }
...
.product-settings-grid > .product-private-voice { grid-column: 1 / -1; padding: 1.25rem; border: 1px solid var(--shell-rule); border-radius: var(--shell-radius-lg); background: var(--shell-surface); box-shadow: var(--shell-shadow-soft); }
.product-private-voice-state { margin-block-end: .7rem; font-weight: 720; }
.product-private-voice[data-private-voice-status="active"] .product-private-voice-state { color: var(--shell-green); }
.product-private-voice[data-private-voice-status="degraded"] .product-private-voice-state { color: var(--shell-amber); }
.product-private-voice[data-private-voice-status="retrying"] .product-private-voice-state, .product-private-voice[data-private-voice-status="switching"] .product-private-voice-state { color: var(--shell-blue); }
.product-private-voice[data-private-voice-configuration-pending] .product-private-voice-state { color: var(--shell-blue); }
...
.product-private-voice-form { display: grid; gap: 1rem; }
.product-private-voice-form fieldset { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: .75rem; margin: 0; padding: .9rem; border: 1px solid var(--shell-rule); border-radius: var(--shell-radius-md); }
.product-private-voice-form legend { padding-inline: .3rem; color: var(--shell-ink); font-size: .9rem; font-weight: 760; }
.product-private-voice-form label { display: grid; gap: .32rem; min-width: 0; color: var(--shell-ink); font-size: .86rem; font-weight: 680; }
.product-private-voice-form input { width: 100%; min-height: 2.75rem; min-width: 0; padding: .55rem .7rem; border: 1px solid var(--shell-rule); border-radius: .7rem; background: var(--shell-bg); color: var(--shell-ink); }
.product-private-voice-form select { width: 100%; min-height: 2.75rem; min-width: 0; padding: .55rem .7rem; border: 1px solid var(--shell-rule); border-radius: .7rem; background: var(--shell-bg); color: var(--shell-ink); }
.product-private-voice-form input:focus { border-color: var(--shell-blue); }
.product-private-voice-form select:focus { border-color: var(--shell-blue); }
.product-private-voice-form small { color: var(--shell-muted); font-size: .77rem; font-weight: 500; }
.product-private-voice-form-status { min-height: 1.3rem; margin: -.25rem 0 0; color: var(--shell-muted); font-size: .85rem; }
.product-private-voice-close { margin: 0; }
.product-private-voice-close p { margin: .1rem 0 .5rem; color: var(--shell-muted); font-size: .84rem; }
...
.product-layout-publication-history { display: grid; gap: .65rem; min-width: 0; padding: 1rem; border: 1px solid var(--shell-rule); border-radius: var(--shell-radius-md); background: var(--shell-surface); }
.product-layout-publication-history h3 { margin: 0; }
...
.product-view-default-list li[data-state="active"] { color: var(--shell-blue); }
.product-view-default-list li > span { display: grid; gap: .08rem; }
.product-view-default-list small { color: var(--shell-muted); font-size: .74rem; }
.product-view-default-list form, .product-view-default-reset { margin: 0; }
.product-view-default-status { color: var(--shell-muted); font-size: .78rem; font-weight: 650; letter-spacing: .01em; }
.product-view-default-reset { grid-column: 2; }
.product-presentation-preferences { display: grid; gap: 1rem; }
.product-presentation-preference { display: grid; grid-template-columns: minmax(0, 1fr) auto; align-items: center; gap: 1rem; padding-block-end: 1rem; border-block-end: 1px solid var(--shell-rule); }
.product-presentation-preference:last-of-type { border-block-end: 0; }
.product-presentation-preference fieldset { min-width: 0; margin: 0; padding: 0; border: 0; }
.product-presentation-preference legend { font-weight: 720; }
.product-presentation-preference p { margin: .22rem 0 0; color: var(--shell-muted); font-size: .82rem; }
.product-presentation-choices { display: flex; flex-wrap: wrap; gap: .5rem; margin-block-start: .7rem; }
.product-presentation-choice { min-height: 2.65rem; display: inline-flex; align-items: center; gap: .42rem; padding: .42rem .72rem; border: 1px solid var(--shell-rule); border-radius: 999px; background: var(--shell-surface); color: var(--shell-muted); font-weight: 680; cursor: pointer; }
.product-presentation-choice:has(input:checked) { border-color: var(--shell-blue); background: var(--shell-blue-soft); color: var(--shell-blue); }
.product-presentation-choice:has(input:focus-visible) { outline: 3px solid color-mix(in srgb, var(--shell-blue) 68%, white); outline-offset: 3px; }
.product-presentation-choice input { margin: 0; accent-color: var(--shell-blue); }
.product-presentation-choice input:focus-visible { outline: 0; }
.product-presentation-reset { margin: 0; }
...
.product-voice-indicator { width: 6rem; height: 6rem; border: .35rem solid color-mix(in srgb, var(--shell-blue) 22%, transparent); border-radius: 50%; background: transparent; box-shadow: inset 0 0 0 .35rem var(--shell-blue); opacity: .82; animation: product-voice-breathe 3.6s cubic-bezier(.25, 1, .5, 1) infinite; animation-play-state: paused; }
.product-voice-status { margin: 0; color: var(--shell-blue); font-size: 1.05rem; font-weight: 760; }
.product-voice-detail { max-width: 48ch; }
.product-voice-privacy { margin: .5rem 0 0; color: var(--shell-subtle); font-size: .78rem; }
...
.product-voice-answer { width: min(100%, 42rem); margin: 0; color: var(--shell-ink); font-size: 1rem; line-height: 1.6; text-align: start; white-space: pre-wrap; }
...
.product-voice[data-voice-state="partial_transcript"] .product-voice-indicator { border-color: color-mix(in srgb, var(--shell-blue) 26%, transparent); box-shadow: inset 0 0 0 .35rem var(--shell-blue); animation-play-state: running; }
...
@media (max-width: 56rem) {
  ...
  .product-settings-grid > .product-private-voice { grid-column: auto; }
  ...
  .product-mobile-nav-link { position: relative; display: grid; justify-items: center; align-content: end; gap: .18rem; min-height: 3.6rem; padding: 1.15rem .15rem .4rem; border-radius: .75rem; color: var(--shell-muted); text-decoration: none; font-size: .7rem; font-weight: 650; }
  ...
  .product-mobile-nav-badges { position: absolute; inset-block-start: .18rem; inset-inline-start: 50%; transform: translateX(-50%); display: inline-flex; gap: .15rem; }
  .product-mobile-nav-badges .product-badge { min-width: 1.05rem; height: 1.05rem; padding-inline: .16rem; font-size: .6rem; }
}
...
@media (max-width: 28rem) {
  ...
  .product-control-space > header { align-items: start; flex-direction: column; }
  .product-control-metrics { width: 100%; justify-content: space-between; }
...
@media (prefers-color-scheme: dark) {
  :root {
    color-scheme: dark;
    --shell-bg: #181b19;
    --shell-surface: #202522;
    --shell-sidebar: #1c211e;
    --shell-ink: #edf1eb;
    --shell-muted: #b5c0b8;
    --shell-subtle: #87938a;
    --shell-rule: #3b463e;
    --shell-blue: #8eb2ff;
    --shell-blue-soft: #202e4c;
    --shell-green: #83c99d;
    --shell-green-soft: #1c3929;
    --shell-amber: #efc174;
    --shell-amber-soft: #463619;
    --shell-red: #f09b90;
    --shell-red-soft: #482522;
    --shell-shadow: 0 16px 40px rgba(0, 0, 0, .22);
    --shell-shadow-soft: 0 6px 18px rgba(0, 0, 0, .16);
  }
}
...
@media (prefers-contrast: more) {
  ...
  .product-media-completion-notification { border-block-end-width: 2px; }
}

@media (prefers-color-scheme: dark) and (prefers-contrast: more) {
  :root { --shell-muted: #d0d8d1; --shell-subtle: #c5cec7; }
}
`;
